...
-   **履歴管理:** 選択したディレクトリの履歴を保存し、後で再利用できます。（UTF-8エンコーディング対応）
...
-   **ドラッグ＆ドロップ:** エクスプローラー等からディレクトリを直接リストにドラッグ＆ドロップして追加できます。
...
-   **ライブラリ:**
    ...
    -   **mod.copy\_support:** コピー処理、リトライ機能、進捗報告
        ...
        -   `mac_linux.py`: macOS/Linux用の`MacLinuxCopy`クラスを提供。`rsync`コマンドを使用してファイルのコピーを実行。プラットフォームに応じて適切なフラグ（macOS: `-aE`, Linux: `-aX`）を使用するように最適化されています。
    ...
    -   pathlib, os, sys, json, logging, xml: 標準ライブラリ (モダンなパス操作のために`pathlib`を活用)
